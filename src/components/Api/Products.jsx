import sportmixsinpasas from '../img/sportmixsinpasas.jpg'
import sportmix from '../img/sportmix.jpg'
import mixmediterraneosinpasas from '../img/mixmediterraneosinpasas.jpg'
import mixmediterraneo from '../img/mixmediterraneo.jpg'
import mixclasicocereales from '../img/mixclasicocereales.jpg'
import mixclasico from '../img/mixclasico.jpg'
import snackmixpremium from '../img/snackmixpremium.jpg'
import snackmixespecial from '../img/snackmixespecial.jpg'
import snackmix from '../img/snackmix.jpg'


export  const products = [
                {
                    id: 1,
                    title: "Sport Mix sin pasas",
                    description: "Nueces, almendras, castañas de cajú, maní, chips de banana",
                    price:"$840/Kg",
                    picture: sportmixsinpasas,
                    stock:10
                },
                {
                    id: 2,
                    title: "Sport Mix",
                    description: "Nueces, almendras, castañas de cajú, maní, chips de banana, pasas de uva morochas",
                    price: "$610/Kg",
                    picture: sportmix,
                    stock:15
                },
                {
                    id: 3,
                    title: "Mix Mediterraneo sin pasas",
                    description: "Nueces, almendras, castañas de cajú, maní",
                    price: "$830/Kg",
                    picture: mixmediterraneosinpasas,
                    stock:10
                },
                {
                    id: 4,
                    title: "Mix Mediterraneo",
                    description: "Nueces, almendras, castañas de cajú, maní, pasas de uva rubias, pasas de uva morochas",
                    price: "$690/Kg",
                    picture: mixmediterraneo,
                    stock:12
                },
                {
                    id: 5,
                    title: "Mix Clásico + Cereales",
                    description: "Nueces, almendras, maní, pasas de uva morochas, copos sin azúcar",
                    price: "$490/Kg",
                    picture: mixclasicocereales,
                    stock:13
                },
                {
                    id: 6,
                    title: "Mix Clásico",
                    description: "Nueces, almendras, maní, pasas de uva morochas",
                    price: "$510/Kg",
                    picture: mixclasico,
                    stock:10
                },
                {
                    id: 7,
                    title: "Snack Mix Premium",
                    description: "Nueces, almendras, castañas de cajú, pasas de uva rubias, pasas de uva morochas",
                    price: "$790/Kg",
                    picture: snackmixpremium,
                    stock:20
                },
                {
                    id: 8,
                    title: "Snack Mix Especial",
                    description: "Nueces, almendras, pasas de uva rubias, pasas de uva morochas",
                    price: "$740/Kg",
                    picture: snackmixespecial,
                    stock:18
                },
                {
                    id: 9,
                    title: "Snack Mix",
                    description: "Nueces, pasas de uva rubias, pasas de uva morochas",
                    price: "$680/Kg",
                    picture: snackmix,
                    stock:19
                },
                
            ]



